...
|---|---|---|
| A green claim must be evidenced | the gate reads a recorded receipt, not the diff; no receipt is a refusal | nothing prevents it |
| A failed run can't be signed green | a receipt whose exit code is non-zero cannot be gated `PASS` (`R:GREENLIE`) | nothing prevents it |
| Evidence can't go stale unnoticed | a receipt recorded before the last edit to the frozen direction is refused | nothing prevents it |
| Every rule must be checked | a rule or edge case with no check bound to it by `covers:` blocks the gate | convention only |
| Security findings can't be waved through | a security-floored node cannot record `RISK-ACCEPTED` (`R:SECURITYFOLD`) — resolve it or `HARD-STOP` | convention only |
| Someone must be on record for security | a security `PASS` without a named lens is refused (`R:NOCOVERAGE`) | not in scope |
| Work produces a verdict | every verify records `PASS` / `RISK-ACCEPTED` / `HARD-STOP` — an audit trail | produces specs, not verdicts |
| The human signature has defined seams | two: the human freezes the direction and owns the verdict at the gate | undefined |
| Resume is a primitive | `add status` is the resume point; state lives on disk | re-read the documents |
| Spec evolution is an operation | five living specs + recorded deltas, folded forward (`add deltas`, `add fold`) | per-feature docs that drift |
| The method adapts to the project | personas are assigned as recorded lenses (`add advise`, `add wave`), and `add learn` records what the loop learned against evidence | static templates |
| Team & lifecycle layer | git-native multi-user, waves over worktrees, components, release gates | not in scope |
...
2. **Hostile or tempting changes** — where the cheapest green is weakening your
   own tests; the refusals exist for exactly the day conventions fail.
...
**Resolution (one release later):** the diagnosis became method — a task in a
foreign repo declares the host suite as one of its own `## CHECKS`, so the gate
binds it like any other — and the re-run scored haiku+ADD at **3/3 resolved, cheaper
