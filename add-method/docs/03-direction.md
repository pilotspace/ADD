...
4. **The assumptions** — recorded in their own `## ASSUMPTIONS` section: what the request did **not** say, the reading you took, and what it costs if that reading is wrong. A node with genuinely nothing uncertain still names its single biggest risk — the AI never claims a blank mind.
   ...
   **Sweep it; don't free-associate.** Author `gives:` first — the `S<n>` surfaces this node publishes — then tag each assumption line with one dimension and the surfaces it covers: `- A<n> [who] covers: S1, S3 · …`. The dimensions are `who` (identity · authority · scope), `which` (inclusion · visibility), `when` (boundaries · timing), `absent` (missing values · defaults), `order` (sequencing · ties) and `experience` (audience · difficulty). Take each surface and ask all six. The last one is the only one that is not about correctness: `who` asks whose data it is and which caller may act, `experience` asks who *receives* the output and what would make it hard for them — answer one and the other is still open. Retire one that cannot apply with `[<dim>] n/a · <why>` — recording a considered *no* is the work; what is refused is silence.
