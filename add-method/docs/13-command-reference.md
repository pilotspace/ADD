...
| `refute` | record that someone tried to refute a green — who tried to break it, against which receipt, what they found (NO-EXEC; no verdict). `--by` required; one of `--held` or `--found "<input>"`; `--probes N`, `--note` | `add refute reject-overlap --by "fresh:verifier" --held --probes 3` |
...
| `locate` | reverse lookup — which node's `scope:` owns a path; `done` owners are counted, `--all` lists them | `add locate src/bookings/service.py --all` |
| `todo` | the open worklist — active tasks grouped by beat. `--milestone` restricts to one | `add todo --milestone auth-layer` |
| `show` | read ONE node whole — its frontmatter, its entire body, and its neighbourhood walked to `--expand` levels (default 3, max 5) across both edge families and both directions. A ref that names no node, or names several, REFUSES and never falls back to a search; an `--expand` above the cap refuses rather than clamping `--json` emits the pinned machine payload (FORMAT.md §11) alone on stdout, byte-stable across runs; a refusal is a payload too and keeps its exit code. | `add show okf-graph-lookup --expand 2` |
| `search` | find a concept anywhere in the bundle — a spec delta answers with its own address (`/specs/method.md#M28`), not with the file that holds thirty of them. `--as-of YYYY-MM-DD` reports each delta under the status it HELD THEN. Also selects by field: `--type` (a type outside the taxonomy REFUSES rather than answering zero hits), `--status` (a node carrying no `status:` matches none), `--milestone` (a bare slug or a cid). Filters AND together and the query becomes optional, but an ask naming NOTHING still refuses. A node-scoped filter cannot judge a delta, so delta hits are excluded with the count reported `--json` emits the pinned machine payload (FORMAT.md §11) alone on stdout, byte-stable across runs; a refusal is a payload too and keeps its exit code. | `add search --type Task --status direction --milestone okf-graph-lookup` |
