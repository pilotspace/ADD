...
CONTRACT (prose-only, presentation not policy — no gate added, none removed):
  ...
  gate-udd.md gains the **Batch, don't serialize** hard rule — N same-gate
...
class IntakeFreezeBatchTest(unittest.TestCase):
    ...
    def test_report_template_batch_rule(self):                 # scenario 3
        text = _trees("gate-udd.md")[0].read_text(encoding="utf-8")
    ...
    def test_tree_parity(self):                                # scenario 5
        for name in ("intake.md", "gate-udd.md"):
    ...
    def test_pools_absorbed(self):                             # scenario 6 — net ≤0B per pool
        ...
        ref = ["scope.md", "deltas.md", "fold.md", "release.md", "gate-udd.md",
        ...
        self.assertLessEqual(total, 51885, "reference pool must absorb the gate-udd addition")
