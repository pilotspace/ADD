...
CONTRACT — supersedes ground-anchor-sha's "fast lane omits it" scope note (recorded change):
  templates/TASK.fast.md.tmpl §0 gains a `Ground SHA:` line (all 3 template trees);
  _FALLBACK_TASK_FAST §0 carries the same field (circuit-breaker parity of the FIELD, not bytes);
  a scaffolded `new-task --fast` TASK.fast.md contains it;
...
import add
from add_engine.constants import _FALLBACK_TASK_FAST

TOOLING = Path(add.__file__).resolve().parent
ADD_METHOD = TOOLING.parent
REPO = ADD_METHOD.parent
FAST_TREES = (
    TOOLING / "templates" / "TASK.fast.md.tmpl",
    REPO / ".add" / "tooling" / "templates" / "TASK.fast.md.tmpl",
    ADD_METHOD / "src" / "add_method" / "_bundled" / "tooling" / "templates" / "TASK.fast.md.tmpl",
)
    ...
    def test_fast_template_has_ground_sha_3trees(self):          # scenario 1
        for p in FAST_TREES:
            self.assertIn("Ground SHA:", _ground_block(p.read_text(encoding="utf-8")),
                          f"{p} §0 must carry the Ground SHA drift anchor")

    def test_fallback_has_ground_sha(self):                      # scenario 2
        self.assertIn("Ground SHA:", _ground_block(_FALLBACK_TASK_FAST),
                      "_FALLBACK_TASK_FAST §0 must carry the field (circuit-breaker parity)")
