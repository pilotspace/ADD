"""learn() must emit the frozen delta grammar (deltas.md §grammar), open by default.
    ...
    - [<COMPETENCY> · <ID> · open · <valid-from>] <learning> (evidence: <pointer>)
...
RE-AIMED by `dated-addressable-deltas`: the head opened to carry the lesson's address and the
start of its validity interval. The CLAIM is unchanged — `learn` emits the frozen grammar, `open`,
with evidence closing it — so the pattern moved and nothing was relaxed.
"""
import re
import sys
...
def test_learn_writes_open_tagged_grammar(tmp_path):
    """covers: M1 — a delta matches the frozen [COMP · ID · open · date] grammar, evidence closing it."""
    add.init(tmp_path, "code", "T")
    ok, _ = add.learn(tmp_path, "method", "budgets need a unit", evidence="/runs/1.md")
    assert ok is True
    assert re.search(r"^- \[ADD · M1 · open · \d{4}-\d{2}-\d{2}\] budgets need a unit "
                     r"\(evidence: /runs/1\.md\)\s*$",
...
def test_competency_derives_from_lens(tmp_path):
    ...
    assert re.search(r"^- \[DDD · D1 · open · [\d-]+\] d-lesson ", _spec(tmp_path, "domain"), re.M)
    assert re.search(r"^- \[TDD · Q1 · open · [\d-]+\] q-lesson ", _spec(tmp_path, "quality"), re.M)
