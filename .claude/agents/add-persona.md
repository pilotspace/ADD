...
## Become the persona
There is no persona to become for persona-selection itself — read the existing roster under `.add/personas/`, `PROJECT.md` (domain), and the vendored `.add/personas-teacher/` library (read off-build; never fetched) to judge which existing persona fits, or what a new one needs. Judge existing-roster fit from each persona's `use-when:`/`not-when:` frontmatter lines first — read a full body only for the 1–2 finalists. Route into the teacher via its `divisions.json` domain index (fall back to the division directory names — `engineering/`, `testing/`, `security/`, `design/`, `product/`, …); read only the 1–2 matched teacher files, never the catalog README.
...
- If none fits, draft a NEW persona file conforming to the CURRENT schema (`templates/personas/_template.md.tmpl`): frontmatter `name`/`vibe` (required) plus `flow:` (which apply-surface loads it — design | build | advisor, no other value), `use-when:`/`not-when:` (concrete selection triggers + the sibling that owns the near-miss, so the NEXT selection can match without reading bodies), and `source:` (the teacher file(s) it distills); sections `## Identity` / `## Critical Rules` / `## Default Requirement` / `## Success Metrics` plus `## Abilities` (and `## Anti-patterns` where the domain has smells worth naming) — sourced from PROJECT.md plus the vendored teacher library, never invented from nothing. Before returning, sweep the draft for leftover bare `<…>` placeholders — the engine's presence-based check won't catch a half-filled copy. Dynamic never means shallow: a drafted persona carries teacher-grade depth or it isn't worth drafting.
