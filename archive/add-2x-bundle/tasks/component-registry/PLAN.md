...
- [ADD · folded] a sentinel value used in logic (`"?"`) must be reserved from any user-supplied namespace it shares (TOML component names) or it silently collides (evidence: refute-read MINOR, fixed by reserving `"?"` + `test_reserved_question_mark_name_is_malformed`). [folded foundation-version 50]
