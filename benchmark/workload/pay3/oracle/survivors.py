"""PAY3 must-survive invariants — the auth/rate-limit LAWS that outlive pay4.

Re-run against the pay4 workspace (resilient settlement + input hardening).
...
def test_reads_stay_open():
    with running_app(_workspace()) as base:
        ...
        assert status == 200 and records(body) is not None
