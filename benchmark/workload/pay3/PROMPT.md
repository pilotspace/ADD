# Payments Milestone 3 — API keys & per-key token-bucket rate limiting

A cross-cutting change to the payments app from Payments Milestone 2. Every
**mutating** request must now be authenticated and rate limited **per API key**.
All Payments Milestone 1–2 behavior (accounts, split transfers, invoices,
