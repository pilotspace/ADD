"""PAY1 FROZEN requirement checklist — the deterministic fidelity of record.

One row per enumerated requirement in pay1/PROMPT.md. `compute_coverage_detail`
...
def _p_idempotent_replay(base, ws):
    ...
    hdr = {"Idempotency-Key": "pay1-replay"}
...
def _p_idempotent_conflict(base, ws):
    ...
    if not src or not payees:
        return False
    hdr = {"Idempotency-Key": "pay1-conflict"}
...
def _total_balance(base) -> int | None:
    ...
    accts = records(accts)
    if status != 200 or accts is None:
        return None
