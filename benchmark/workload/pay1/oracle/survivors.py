"""PAY1 must-survive invariants (family-local regression, mirrors the wm track).

Regression probes re-run against EVERY LATER PAY workspace — so each probe must
hold under legitimate later evolution (pay2 adds a pricing/invoice layer; pay3
adds a per-key token-bucket rate limit; pay4 adds resilient settlement + input
hardening). Two rules keep them survivable:

1. KEY-CARRYING: every call sends the pinned API key (`bench-key-alice`, pinned
   in pay3's PROMPT). An pay1/pay2 app ignores the header; an pay3+ app requires it.
   Calls stay well under the pay3 bucket (capacity 60) so a later app never 429s
