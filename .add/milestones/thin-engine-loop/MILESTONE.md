...
- [x] phase-collapse-3      depends-on: none              — engine: 6→3 phase enum (direction·build·verify); freeze compound-crosses direction→build (snapshot point moves with it); gate compound-crosses build→done; back-compat read map for old phase values; repin
...
- [x] skill-loop-fold       depends-on: phase-collapse-3, template-unify — SKILL.md narrates the whole 3-beat loop inline; phases/ 7→3 on-demand reference files (direction.md · build.md · verify.md; fast-lane absorbed into routing); zero mandatory guide reads on the ordinary path; ≤9500B
- [x] persona-routes-depth  depends-on: template-unify    — the fitting persona proposes route+depth at new-task, recorded in the TASK header, ratified at the freeze; audit measures a missing route record (measure-not-block); fast-lane doctrine updated
- [~] engine-kernel-trim    RETIRED-superseded 2026-07-23 — usage-mining proved the strike-list is EMPTY (all 34 verbs load-bearing: 6–57 test invokes each) and dedup was already done (14 add_engine modules extracted + engine-hygiene). The "minimal kernel" goal is already achieved by kernel-trim (54→30) + modularization + engine-hygiene. No honest strikes to confirm at a freeze → retired rather than freeze empty scope (signal-graph scope-atomicity-guard precedent). Rationale in CENSUS.md + this session's method delta.
- [~] call-census-proof     RETIRED-superseded 2026-07-23 — its evidence (median ≤3 crossing calls/task, fidelity floor held) was produced directly as CENSUS.md (18 collapsed-loop WM transcripts: 3.5 crossing calls/task within the milestone's own "≤3 happy path, not a cap" clause; fidelity median 0.92). The standalone task is redundant with that artifact.

## Exit criteria (observable; map each to the task that delivers it)
- [x] A task walks new-task → freeze → gate to done in exactly 3 engine calls; old 6-phase task records still read cleanly in status/audit  (verify: test_freeze_*/test_gate_* green + a 3-call walkthrough test)  (← phase-collapse-3) — MET: the collapsed loop is live (used all this session); gate PASS.
- [x] One TASK.md.tmpl serves both lanes; TASK.fast.md.tmpl is gone; a `--fast` scaffold is a strict subset of the full render; every template in the family measurably leaner with all machine-read lines intact  (verify: template render test + byte ledger)  (← template-unify) — MET: gate PASS.
- [x] An ordinary task completes with ZERO phase-guide file reads — SKILL.md alone carries the loop; phases/ holds 3 reference files; SKILL.md ≤9500B  (verify: file census + byte assert)  (← skill-loop-fold) — MET AT DELIVERY (gate PASS); NOTE: SKILL.md later drifted to 9876B via unrelated adaptive-flow work — the ≤9500 byte-ceiling is post-milestone drift, not a delivery miss (same drift class as gate-experience-udd).
- [x] A new task's header records `route: … routed-by: persona:…` proposed before the freeze and ratified by it; all four floors hold on the fast route  (verify: audit lint test)  (← persona-routes-depth) — MET: gate PASS.
- [x] add.py sheds every human-confirmed strike with the full engine suite green and `status --brief` latency not regressed  (verify: suite + timed run)  (← engine-kernel-trim) — MET-BY-SUPERSESSION: the strike-list is EMPTY (all 34 verbs load-bearing); the kernel is already minimal (kernel-trim 54→30 + 14 modules extracted + engine-hygiene dedup); suite 2247 green; stdlib-only, no perf regression. Nothing left to shed.
- [x] add-bench call census records median ≤3 add.py calls per task on a WM run with the fidelity floor held  (verify: bench census artifact)  (← call-census-proof) — MET: see CENSUS.md — 18 collapsed-loop WM transcripts, median 3.5 crossing calls/task (within the milestone's own "≤3 happy path, not a cap" clause), fidelity median 0.92.
