...
## EXIT
- [x] a relation is identified by the lesson that declared it: two lessons refining one target emit two rows, and the payload names which lesson each came from   (relation-identity-in-the-walk)
- [x] the walk's edge count on the live bundle equals what relations() and edges() together report, proven by comparing the two rather than by pinning a number   (relation-identity-in-the-walk)
- [x] the four checks that could not fail now fail when their defect is injected: the cap's value, the refusal's verb, the cache independence, and the payload's present fields   (checks-that-cannot-fail)
- [x] the CLI's --expand default and the engine's NEIGHBORHOOD_DEFAULT are compared by a check that reads both declarations   (checks-that-cannot-fail)
- [x] a filename that names exactly one node resolves to it, and an ambiguity refusal is bounded the way the depth cap is bounded   (ref-resolution-accepts-what-an-operator-types)
