...
- [x] kickoff-truth       depends-on: none            — engine message layer: lane-aware kickoff (oneshot-first for single-task work) · new-task emits the FULL remaining call recipe for its lane · `--to` hand-off names its fill-first precondition · 2nd byte-identical failing call short-circuits with a change-something-first hint. GROUND FIRST: re-verify which transcript baits survive the current engine.
- [x] derived-stamps      depends-on: none            — engine auto-stamps Ground SHA · Status DRAFT→FROZEN · Reported: · GATE dates; template fields become engine-filled placeholders (fast twin parity incl. the Ground-SHA inconsistency).
- [x] template-dedup      depends-on: derived-stamps  — TASK.md.tmpl: 11 EXIT comments (~2.9KB, 23%) → one-line pointers to the phase guides' own exit_gate blocks; single-source the 5 rules currently stated 3-5× across SKILL.md/guides/template.
- [x] gate-read-diet      depends-on: none            — report-template.md + run.md become read-once-per-session references; each gate's ~20 essential lines fold into its phase guide as a slim card; SKILL.md orient + flow-table trim (drop the Produces column).
- [x] scope-echo-draft    depends-on: none            — freeze report renders the RESOLVED scope token list (mis-resolution becomes a zero-call read at the approval already happening); Scope line auto-drafted from §3 Grounding Touches, propose-not-impose, grammar untouched.
- [x] risk-report-render  depends-on: gate-read-diet  — compact gate render for sensitivity:mechanical/fast lane; the full 8-section render reserved for security/data/architecture + the freeze.
- [x] fold-draft-at-close depends-on: none            — milestone-done auto-DRAFTS the fold (deltas grouped, seed/drop pre-classified with one-line rationale); human still confirms the fold — no gate change.
...
- [x] pinned-meter WM1 re-measure (n=3): mean add.py calls ≤ 12 · zero --help turns · zero duplicate-identical failing calls · turns/cost ≤ 77.7t/$2.97 · oracle fidelity held · scope_violation count 0 — **RESOLVED 2026-07-23 by the signed close-on-trend waiver (Tin Dang)**: two further paid runs landed 14.3 then 13.3 mean (trend 27→13.3, −51%), fidelity 1.00 on all 6 reps, the strict ≤12 mean waived; details .add/benchmark-remeasure-2026-07-23.md   (← all; RAN 2026-07-13, $10.53 — benchmark/results/2026-07-ceremony-remeasure.md: calls 18.7 mean UNMET (−31% vs baseline-round3's 27) · --help 0.7 near-zero · dup 1.7 near-zero · 134t/$3.51 UNMET on this meter yet the cheapest honest ADD round ever archived (−32%t/−44%$ vs baseline-round3) · fidelity 0.98 HELD, 0 regressions · residual anatomy names the next levers: double-init, milestone bait, re-cross repairs, status re-reads. Bar unmet → box stays open for the human's accept-partial or follow-on call)
...
- [x] each Exit criterion above is satisfied by a Cross-task evidence row or a Ship-by-domain change (cite which) — 7/9 test-pinned inline; read-burden closed as residue (56→37-49KB, ≤30KB tail = open backlog) and the pinned-meter criterion resolved by the 2026-07-23 signed waiver
