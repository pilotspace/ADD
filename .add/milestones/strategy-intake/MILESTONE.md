...
goal: A fitting persona becomes the project-management brain — it shapes each milestone's strategy AND owns how every human gate is communicated and paced, replacing the fixed report-template ceremony. Personas adapt per project; one floor stays hard — security is always HARD-STOP.
...
- [ ] persona-owns-gates      depends-on: none                                    — retire the fixed report-template section list; personas own gate structure + cadence; report-template.md becomes persona-adaptive PRINCIPLES carrying the 3 in-contract floors + the security carve-out. The contract the rest cites
...
- [ ] Gate reports are persona-owned — structure + cadence adapt per project; no fixed template section list; the 3 floors are met in the persona's voice and security stays HARD-STOP  (← persona-owns-gates)
