...
## Deltas (newest first)
- [open · 2026-07-27] a guidance change that only reaches the benchmark wrapper improves the number without improving the method — every instructional change must land in the shipped guide FIRST and reach the arm second, and the arm's controls must be asserted clause-free (evidence: benchmark/tests/test_read_batching_wrapper.py::test_raw_wrapper_is_untouched) (task:read-batching)
