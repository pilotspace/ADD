...
test_cmd: python3 -m pytest add-method/tests/engine/test_evidence_and_lessons_views.py -q -p no:cacheprovider --junitxml=/var/folders/1j/dpmnj2ns0l9b36b65zjktclc0000gn/T//add-run.xml
---
...
- [a check holds where CI runs it, or it does not hold](milestones/checks-that-hold-in-ci.md) — `done`
- [Dynamic Flow — adaptive path, deterministic trust](milestones/dynamic-flow.md) — `done`
- [Evidence over tests — trust on independent evidence against frozen intent](milestones/evidence-over-tests.md) — `done`
...
- [the learning loop converges instead of accumulating](milestones/loop-that-drains.md) — `done`
...
- [a persona loads by fit, on the path that does not spawn](milestones/personas-load-by-fit.md) — `done`
...
- [the loop refuses in one voice, and early](milestones/refuse-in-one-voice.md) — `done`
...
- [the rules we bound today hold for our own tree](milestones/rules-that-hold-for-us.md) — `done`
...
- [a guard reading a git ref says whether it is durable or a tripwire](tasks/a-head-guard-declares-its-lifetime.md) — `done` · `standard`
- [a kind is from the closed taxonomy](tasks/a-kind-is-from-the-taxonomy.md) — `done` · `quick`
- [a scaffold says whether a plan still wants it](tasks/a-plan-says-what-it-wants.md) — `done` · `standard`
- [a roadmap of queued tasks is legible without opening every file](tasks/a-roadmap-reads-as-a-roadmap.md) — `done` · `standard` · `architecture`
...
- [the 78 open deltas resolve to folded, rejected or bound](tasks/backlog-drain.md) — `done` · `quick`
...
- [brief names the roster personas that fit, and selects none](tasks/brief-names-the-candidate-lens.md) — `done` · `standard`
...
- [a milestone close drains its own lessons, and a drained lesson can bind](tasks/deltas-drain-at-close.md) — `done` · `standard` · `architecture`
...
- [dogfood evidence-over-tests on real work and decide whether a bench is warranted](tasks/dogfood-and-measure.md) — `done`
...
- [EVIDENCE and LESSONS are written by the engine — a done node never carries the scaffold](tasks/evidence-and-lessons-are-views.md) — `done` · `standard` · `architecture`
...
- [freeze refuses an authored edge or probed assumption no check covers](tasks/freeze-binds-what-you-authored.md) — `done` · `standard` · `architecture`
...
- [a milestone human freeze needs proof the exit criteria were put to a human](tasks/milestone-freeze-is-interviewed.md) — `done` · `standard` · `architecture`
...
- [the search-filter suite pins its clock and still catches the regression](tasks/no-check-fails-by-calendar.md) — `done` · `quick`
...
- [one budget, one guard](tasks/one-budget-one-guard.md) — `done` · `quick`
- [the SKILL.md sha256 pin has one home, like the budgets](tasks/one-home-for-the-prose-pin.md) — `done` · `standard`
- [one placeholder oracle, one covers grammar, and no doc that contradicts the engine](tasks/one-oracle-one-truth.md) — `done` · `standard` · `architecture`
- [freeze refuses with one falsy shape](tasks/one-refusal-shape.md) — `done` · `quick`
- [status and doctor see carried work; the root is authored](tasks/orientation-sees-carried-work.md) — `done` · `standard` · `architecture`
...
- [the beat guides select a lens by fit, not the spawn](tasks/persona-loads-on-every-path.md) — `done` · `standard`
...
- [every verb refuses with None](tasks/refuse-with-one-shape.md) — `done` · `standard`
- [R:UNREFUTED and R:REFUTED — a PASS at a plan-or-higher floor needs a refute recorded after the gated run](tasks/refute-gate-rung.md) — `done` · `standard` · `architecture`
- [add refute — the refute-read becomes a stamp the gate can order, and filled edges join the interview](tasks/refute-verb.md) — `done` · `standard` · `architecture`
...
- [a scope guard names the range it guards](tasks/scope-guard-names-its-range.md) — `done` · `quick`
...
- [status answers what needs me](tasks/status-answers-what-needs-me.md) — `done` · `standard`
- [the explore scaffold is an explore](tasks/the-explore-scaffold-is-an-explore.md) — `done` · `standard`
- [a refusal message is pinned to content, not to a diff](tasks/the-message-pin-is-a-content-pin.md) — `done` · `standard`
...
- [freeze refuses an uncovered Must](tasks/uncovered-widens-to-rules.md) — `done` · `standard`
...
- [the shipped-behaviour lens — a feature is what someone else can now do](personas/feature-builder.md) — a task adds or changes a user-facing behaviour — a verb, a flag, a surface, a refusal someone will read, a contract another node will `need:`
