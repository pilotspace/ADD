...
train-only transform not fit on the test split) are the **composer's** job —
...
| `preprocess_leakage` | a `train_only_transforms` entry is fit on rows in the test partition |
...
| `metric_unpinned` | the primary metric/threshold was added or changed after the contract freeze |
...
| `io_compat_break` | the contract version changed with an undeclared breaking schema diff |
...
| `eval_train_contamination` | an eval case appears (by hash/near-dup) in a declared finetune/training set |
| `eval_set_mutated` | a frozen test row changed after the tests→build snapshot (routes through `build_tampered`) |
...
| `overfit_gap` | the recorded train-vs-test gap exceeds tolerance, or test ≥ train by a margin (leakage-suspect) |
...
- **No leakage, ever.** `split_overlap`, `group_leakage`, `duplicate_leakage`,
  `preprocess_leakage`, `selection_on_test`, `eval_train_contamination` are
  **HARD-STOP class** — never `RISK-ACCEPTED`. A leaked eval is non-evidence, like a
  weakened test.
- **Never weaken the frozen eval.** Lowering a threshold, mutating a frozen test row
  (`eval_set_mutated`), swapping in a lenient judge, or weakening `rubric.json` routes
  through `build_tampered` — a change request back to **Specify**, never launderable
  through `RISK-ACCEPTED`.
...
- **A safety/guardrail finding is always a HARD-STOP** — prompt-injection (incl.
  ...
  missing toxicity guardrail, or unsafe action folds into ADD's
