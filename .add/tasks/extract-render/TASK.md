...
### Competency deltas
- [ADD · open] a SHARED constant (used by both moving + staying code) relocates to constants.py as the single source — distinguish from a cluster-PRIVATE const (travels with the cluster, like _ANSI/_INIT_EXCLUDE) (evidence: _DEFAULT_WIDTH vs _ANSI)
