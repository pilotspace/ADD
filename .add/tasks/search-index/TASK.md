...
- [SPEC · seeded] confirm with Tin whether a task's declared `Glossary deltas` (§3) propagating into `.add/GLOSSARY.md` is genuinely deferred to done/fold across this whole milestone-set (evidence: both `search-index`'s and the sibling `rule-id-coverage`'s declared terms are absent from GLOSSARY.md while both were mid-flight — consistent, but never explicitly confirmed as the intended lifecycle) [→ fold-glossary-deltas]
