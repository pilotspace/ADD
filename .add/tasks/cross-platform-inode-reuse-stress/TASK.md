# TASK: Cross-Platform Inode-Reuse Stress Test

slug: cross-platform-inode-reuse-stress · created: 2026-07-03 · stage: mvp
milestone: (none)
autonomy: auto   <!-- inherited from the project default (PROJECT.md); explicit level: manual < conservative < auto (visible · overridable) — lower below if a high-risk task needs it, or run `add.py autonomy set`. Multi-component repo (monorepo/multi-repo)? add a `component: <name>` line (declared in `.add/components.toml`) to ADD that component's root to your §5 Scope; omit for single-component projects (byte-identical default). -->
phase: specify   <!-- specify -> scenarios -> plan -> tests -> build -> verify -> observe -> done -->
...
Related intent: seeded from global-lock-followups + project-scope-install-lock spec-deltas — the identity-verified-reclaim safety argument rests on documented API contract, not cross-platform measurement; both sibling lock tasks flagged this identically [← global-lock-followups, project-scope-install-lock]
...
Feature: independently stress-test the ticket/lock identity-verified reclaim's inode-reuse assumption on Linux and Windows — this session's 1167+ adversarial attempts (across both `_update_lock` and `_project_lock`) all ran on macOS/APFS; inode reuse timing and semantics differ by filesystem (from global-lock-followups + project-scope-install-lock spec-deltas, an identical ask from both sibling lock tasks)
