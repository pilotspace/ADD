...
- [SPEC · carried] the merged header does not de-dup repeated entries in `active_milestones` (a leftover dup would read `merged: m1 + m1 (2 milestones)`) — cosmetic only; guard if `active_milestones` is ever observed to carry dups (evidence: de-dup is only against the primary today, this task) [carried: deferred to backlog 2026-06-27 (delta-drain) — archived-task delta, not now-actionable; retrievable via 'add.py deltas --carried', reopen/seed via 'new-task --from-delta' when scheduled]
