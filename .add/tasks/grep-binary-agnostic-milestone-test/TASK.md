...
Related intent: seeded from add-advisor + skill-tree-compaction-audit spec-deltas — a pre-existing BSD-vs-GNU `grep -cl` local-only failure (green on CI) independently disclosed by 2 separate tasks [← add-advisor, skill-tree-compaction-audit]
