...
- [ADD · folded] reuse one predicate across gates by EXTENDING it conservatively (any-header break + skip `>` guidance) and prove the prior caller's truth table still holds (evidence: _section_unfilled shared by contract-fill + build-expectations; test_contract_fill_gate 7/7 stayed green) [folded foundation-version 47]
