...
### Spec delta
- [SPEC · open] a whitespace-padded inline literal-comment example (`` `  ` ``) is not protected by the current fix — theoretical only (does not occur anywhere in this repo's current prose), worth a follow-up if this style is ever adopted (evidence: round-1 add-verify refute-read, agent adfec8f93940fdfe5)

### Competency deltas
- [TDD · open] when narrowing a "protect X from Y" regex fix, run the FULL existing test suite (not just new targeted tests) before considering it green — a first broader fix passed its own new tests but silently regressed an EXISTING, unrelated test (`test_reopen_regate_is_clean`) by fragmenting real comments that happen to contain unrelated backtick-quoted code (evidence: this task's own build attempt 1 vs 2)
