...
- [ADD · folded] a §-section text edit must be SCOPED to that section's span, never a full-document `re.subn count=1` — a bare marker (`Status: DRAFT`) can recur in an earlier section and get hit first; the validate-on-§3 / write-on-full-text split hid it (evidence: refute-read found `cmd_freeze` froze a §1 decoy line; fixed + regression test test_freeze_targets_section3_not_a_decoy_draft_line) [folded foundation-version 55]
- [TDD · folded] a build's own happy-path fixtures can mask a scoping bug when they only ever place the target marker in the right section — an adversarial refute-read that injects a decoy is what caught it (evidence: the 8 original tests were green; the bug surfaced only under the refute-read's decoy-in-§1 attack) [folded foundation-version 55]
