...
Feature: sweep aged orphan `.reclaim-*`/`.reclaim-<inode>` ticket files left by a crashed lock holder under both the global home (`_update_lock`) and project-scope (`_project_lock`) — currently permanent, harmless litter with nothing to clean it up (from global-lock-followups + project-scope-install-lock spec-deltas, an identical ask from both sibling lock tasks)
