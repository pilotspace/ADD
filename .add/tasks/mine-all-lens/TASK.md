...
- [SPEC · open] `mine --all` could grow a `--milestone <slug>` narrowing or a `--stage`/phase filter for a large all-milestone backlog — deferred as scope creep (evidence: this task scoped the boolean widening only)
