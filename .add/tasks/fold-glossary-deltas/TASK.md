...
Feature: extend `add.py fold` to also consolidate every task's declared §3 `Glossary deltas` into `.add/GLOSSARY.md`, mirroring how it already consolidates competency deltas (DDD/SDD/UDD -> PROJECT.md, TDD/ADD -> CONVENTIONS.md via `_FOLD_ROUTES`) -- today `add.py` only ever READS GLOSSARY.md (for sensitivity classes), it never writes a declared term into it anywhere, at fold time or otherwise (from search-index spec-delta; confirmed by Tin Dang, 2026-07-03: Glossary deltas defer to fold, project-wide, same as competency deltas)
